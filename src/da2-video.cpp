/*
  ...
  An example of using the DA2Network class with OpenCV for a video stream by Bruce A. Maxwell
  Edited by: Rishi Patel
  Mostly edited for windows adaptation and making sure the code runs.
*/
...
int main(int argc, char *argv[]) {
  ...
  DA2Network da_net( "C:/Users/rishi/Desktop/CV/Project 1/data/model_fp16.onnx" ); // path to the model for windows adaptation

  // open the video device
  capdev = new cv::VideoCapture(0 + cv::CAP_DSHOW); // CAP_DSHOW is another windows adaptation to work for the camera driver
  if( !capdev->isOpened() ) {
