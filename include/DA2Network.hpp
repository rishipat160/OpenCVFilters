/*
  ...
  Original Author: Bruce A. Maxwell
  Edited by: Rishi Patel
  January 2025
  ...
  Edited by: Rishi Patel
  January 2025
  Edited for windows adaptation and making sure the code runs.
...
/**
 * Had to convert the char* to wchar_t* for Windows, also used getWc per piazza post.
 */
class DA2Network {
private:
...
public:
  ...
  DA2Network(const char* network_path) {
    ...
    // Convert to Windows string type
    std::wstring wpath = GetWc(network_path);
    
    // set up the Ort session
    this->session_ = new Ort::Session(env, wpath.c_str(), Ort::SessionOptions{nullptr});
  }
  ...
  int run_network( cv::Mat &dst, const cv::Size &output_size ) {
    ...
    // get the min and max of the output tensor
    float max = -1e+6;
    float min = 1e+6;
    ...
    const float num_slices = 12.0f; 
    
    // Process each pixel
    for(int i=0,k=0; i<out_height_; i++) {
        unsigned char *ptr = tmp.ptr<unsigned char>(i);
        for(int j=0; j<out_width_; j++, k++) {
            // Normalize depth and apply power curve for more detail in far objects
            float depth = (tensorData[k] - min) / (max - min);
            depth = pow(depth, 0.7f);  // Values < 1 give more detail to far objects
            ptr[j] = (unsigned char)(255.0f * floor(depth * num_slices) / num_slices);
        }
    }
